...
def prune_fn(context: str, query: str) -> PruneResponse:
    ...
    try:
        logging.info(f"[PRUNER] Sending request to pruner service: {pruner_url}")
        ...
        logging.info(f"[PRUNER] Request complete, response type: {type(response)}, is None: {response is None}")
        if response is None:
            logging.error("[PRUNER] Response is None")
        ...
        logging.error(f"Pruner request error ({type(e).__name__}): {str(e)}")
        raise
...
class BashObservation(Observation):
    ...
    original_output: Optional[str] = Field(default=None)
